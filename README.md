...
It now includes a standalone voice-led inventory product at `/gemini`. The route starts one live video frame, speaks the merchant through product inventory capture, extracts inventory with server-side OpenAI vision, stores the prototype capture locally in IndexedDB, and generates a storefront JSON export. Merchant role selection opens this same working product.

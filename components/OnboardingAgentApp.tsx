...
const KYC_PROMPT = "Let's start with your KYC. Please provide some verification by showing your Aadhaar card.";
const INVENTORY_PROMPT =
  "Got it. Let's get your inventory. Please show me and tell me what inventory you have and in what quantity.";
...
export function OnboardingAgentApp() {
  ...
  const kycRef = useRef<KycExtraction | null>(null);
  const inventoryRef = useRef<InventoryItem[]>([]);

  const [stage, setStage] = useState<OnboardingStage>("idle");
  const [isRunning, setIsRunning] = useState(false);
  const [isExtracting, setIsExtracting] = useState(false);
  const [kyc, setKyc] = useState<KycExtraction | null>(null);
  ...
  function rememberFrame(stageName: SourceFrame["stage"], frameBase64: string) {
    setSourceFrames((current) => {
      ...
      const withoutOldKyc =
        stageName === "kyc" ? current.filter((item) => item.stage !== "kyc") : current;
      const next = [...withoutOldKyc, frame]
        .filter((item) => item.stage === "kyc" || item.stage === "inventory")
        .slice(-7);
  ...
  useEffect(() => {
    ...
    if (!kyc && !inventory.length && !transcript.length && !sourceFrames.length) return undefined;
  ...
  }, [buildExport, inventory.length, isRunning, kyc, sourceFrames.length, stage, transcript.length]);

  async function extractFrame(stageName: "kyc" | "inventory", frameBase64: string) {
  ...
  const runKycScan = useCallback(async () => {
    if (extractionInFlightRef.current || stage !== "kyc_scanning") return;
    ...
    try {
      const data = await extractFrame("kyc", frameBase64);
      if (data.kyc) {
        setKyc(data.kyc);
        kycRef.current = data.kyc;

        if (data.kyc.isComplete) {
          rememberFrame("kyc", frameBase64);
      ...
      setError(unknownError instanceof Error ? unknownError.message : "KYC extraction failed.");
    } finally {
  ...
  useEffect(() => {
    if (stage !== "kyc_scanning") return undefined;

    const interval = window.setInterval(() => void runKycScan(), 1500);
    void runKycScan();
  ...
  async function startOnboarding() {
    ...
    try {
      ...
      await voice.start();
      setStage("kyc_prompt");
      voice.speak(KYC_PROMPT);
      window.setTimeout(() => setStage("kyc_scanning"), 1200);
    } catch (unknownError) {
  ...
  return (
    <main className={styles.onboardingShell}>
      <section className={styles.onboardingHeader} aria-labelledby="onboarding-title">
        <div>
          <span className={styles.eyebrow}>Merchant onboarding agent</span>
          <h1 id="onboarding-title">KYC and inventory, captured by voice and video.</h1>
        </div>
      ...
      <section className={styles.onboardingWorkspace}>
        ...
        <aside className={styles.captureSummary} aria-label="Captured data summary">
          <div className={styles.privacyNotice}>
            ...
            <span>Prototype only. Full OCR is stored locally in this browser, not official KYC verification.</span>
          </div>
          ...
          <section>
            ...
            {kyc ? (
              <div className={styles.summaryBlock}>
                <p>{kyc.documentType}</p>
                <dl>
                  <div>
                    <dt>Name</dt>
                    <dd>{kyc.fields.name || "Unknown"}</dd>
                  </div>
                  <div>
                    <dt>Aadhaar</dt>
                    <dd>{kyc.fields.aadhaarNumber || "Unknown"}</dd>
                  </div>
                  <div>
                    <dt>Confidence</dt>
                    <dd>{kyc.confidence.toFixed(2)}</dd>
                  </div>
                </dl>
              </div>
            ) : (
              <p className={styles.emptyCopy}>Waiting for Aadhaar OCR.</p>
            )}
          </section>
